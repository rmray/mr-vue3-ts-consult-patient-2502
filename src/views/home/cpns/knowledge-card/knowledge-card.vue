...
import { useFollow } from '@/hooks/home'
...
// 关注文章
const { loading, hdlFollow } = useFollow('knowledge')
</script>

<template>
  <div class="knowledge-card van-hairline--bottom">
    <div class="head">
      <van-image :src="item.creatorAvatar" class="avatar" round></van-image>
      <div class="info">
        <p class="name">{{ item.creatorName }}</p>
        <p class="dep van-ellipsis">{{ item.creatorHospatalName }} {{ item.creatorDep }} {{ item.creatorTitles }}</p>
      </div>
      <van-button class="btn" round size="small" :loading="loading" @click="hdlFollow(item)">
    ...
    <div class="body">
      ...
      <div class="imgs" :class="{ large: item.coverUrl.length === 1 }">
        <van-image v-for="(url, i) in item.coverUrl" :key="i" :src="url" fit="cover"></van-image>
      </div>
      <p class="logs">
